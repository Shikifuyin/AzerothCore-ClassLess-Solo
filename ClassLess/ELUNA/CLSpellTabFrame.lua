...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessSpellTabFrame - Declaration
ClassLessSpellTabFrame = class({
	-- Static Members
})

function ClassLessSpellTabFrame:init()
	-- Members
...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessSpellTabFrame : Getters / Setters

-------------------------------------------------------------------------------------------------------------------
-- ClassLessSpellTabFrame : Methods
function ClassLessSpellTabFrame:Show()
	if ( not self.m_hFrame:IsVisible() ) then
		self.m_hFrame:Show()
	end
end
function ClassLessSpellTabFrame:Hide()
	if self.m_hFrame:IsVisible() then
		self.m_hFrame:Hide()
	end
end

function ClassLessSpellTabFrame:UpdateButton( iSpecIndex, iSpellIndex )
	-- Get Spells Data
	local hDataSpells = CLClient:GetDataSpells()
	...
	-- Get State
	local bKnown = CLClient:IsSpellKnown( self.m_iClassIndex, iSpecIndex, iSpellIndex )
	local bPending = CLClient:IsSpellPending( self.m_iClassIndex, iSpecIndex, iSpellIndex )
...
function ClassLessSpellTabFrame:Update()
	-- Get Spells Data
	local hDataSpells = CLClient:GetDataSpells()
...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessSpellTabFrame : Initialization
function ClassLessSpellTabFrame:Initialize( hParentFrame, iClassIndex )
			...
			self.m_arrSpellButtons[iSpecIndex][iSpellIndex]:SetScript( "OnClick",
				function( self, strMouseButton, bHeldDown )
					...
					if ( strMouseButton == "LeftButton" ) then
						CLClient:AddPendingSpell( hThis.m_iClassIndex, iSpecIndex, iSpellIndex )
					elseif ( strMouseButton == "RightButton" ) then
						CLClient:RemovePendingSpell( hThis.m_iClassIndex, iSpecIndex, iSpellIndex )
					end
