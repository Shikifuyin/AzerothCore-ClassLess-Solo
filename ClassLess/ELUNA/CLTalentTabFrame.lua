...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessTalentTabFrame - Declaration
ClassLessTalentTabFrame = class({
	-- Static Members
})

function ClassLessTalentTabFrame:init()
	-- Members
...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessTalentTabFrame : Getters / Setters

-------------------------------------------------------------------------------------------------------------------
-- ClassLessTalentTabFrame : Methods
function ClassLessTalentTabFrame:Show()
	if ( not self.m_hFrame:IsVisible() ) then
		self.m_hFrame:Show()
	end
end
function ClassLessTalentTabFrame:Hide()
	if self.m_hFrame:IsVisible() then
		self.m_hFrame:Hide()
	end
end

function ClassLessTalentTabFrame:UpdateButton( iSpecIndex, iGridTier, iGridSlot )
	-- Get Talents Data
	local hDataTalents = CLClient:GetDataTalents()
	
	-- Get Talent Desc
	local hTalentDesc = hDataTalents:GetTalentDesc( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )

	-- Get State
	local bKnown = CLClient:IsTalentKnown( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
	local bPending = CLClient:IsTalentPending( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
	
	local iSpecAllocatedTalentPoints = CLClient:GetSpecAllocatedTalentPoints( self.m_iClassIndex, iSpecIndex )
	local iRequiredTalentPoints = CLClient:GetRequiredTalentPoints( iGridTier )
	...
	if ( iRequiredTalentGridTier ~= 0 and iRequiredTalentGridSlot ~= 0 ) then
		...
		-- Must be present
		local iRequiredTalentIndex = CLClient:GetPendingTalentIndex( self.m_iClassIndex, iSpecIndex, iRequiredTalentGridTier, iRequiredTalentGridSlot )
		if ( iRequiredTalentIndex ~= 0 ) then
			hRequiredTalentDesc = CLClient:GetPendingTalentDesc( iRequiredTalentIndex )
		else
			iRequiredTalentIndex = CLClient:GetKnownTalentIndex( self.m_iClassIndex, iSpecIndex, iRequiredTalentGridTier, iRequiredTalentGridSlot )
			if ( iRequiredTalentIndex ~= 0 ) then
				hRequiredTalentDesc = CLClient:GetKnownTalentDesc( iRequiredTalentIndex )
			end
		end
		-- Must be maxed
	...
	if bPending then
		local iIndex = CLClient:GetPendingTalentIndex( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
		iCurrentCost = CLClient:GetPendingTalentDesc(iIndex):GetCurrentCost()
	elseif bKnown then
		local iIndex = CLClient:GetKnownTalentIndex( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
		iCurrentCost = CLClient:GetKnownTalentDesc(iIndex):GetCurrentCost()
	end
	self.m_arrTalentButtons[iSpecIndex][iGridTier][iGridSlot].counter.text:SetText( iCurrentCost )
end
function ClassLessTalentTabFrame:Update()
	-- Get Talents Data
	local hDataTalents = CLClient:GetDataTalents()
...
-------------------------------------------------------------------------------------------------------------------
-- ClassLessTalentTabFrame : Initialization
function ClassLessTalentTabFrame:Initialize( hParentFrame, iClassIndex )
					...
					self.m_arrTalentButtons[iSpecIndex][iGridTier][iGridSlot]:SetScript( "OnEnter",
						function()
							...
							if hTalentDesc:IsTalentSpell() then
								...
								hToolTip:AddLine( "TALENTID: " .. iTalentID, RED_FONT_COLOR )
							else
								-- Get appropriate rank
								local iRank = 0
								local iIndex = CLClient:GetPendingTalentIndex( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
								if ( iIndex ~= 0 ) then
									iRank = CLClient:GetPendingTalentDesc(iIndex):GetCurrentRank()
								else
									iIndex = CLClient:GetKnownTalentIndex( self.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
									if ( iIndex ~= 0 ) then
										iRank = CLClient:GetKnownTalentDesc(iIndex):GetCurrentRank()
									end
								end
								if ( iRank == 0 ) then
									iRank = 1
					...
					self.m_arrTalentButtons[iSpecIndex][iGridTier][iGridSlot]:SetScript( "OnClick",
						function( self, strMouseButton, bHeldDown )
							...
							if ( strMouseButton == "LeftButton" ) then
								CLClient:AddPendingTalent( hThis.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
							elseif ( strMouseButton == "RightButton" ) then
								CLClient:RemovePendingTalent( hThis.m_iClassIndex, iSpecIndex, iGridTier, iGridSlot )
							end
